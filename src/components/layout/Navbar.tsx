...
import { LogIn, UserPlus, User, Settings, LogOut, Store, Menu, X, MessageCircle } from "lucide-react";
...
export function Navbar() {
  ...
  const { unreadMessages, unapprovedRequests } = useNotifications();
          ...
          <div className="flex items-center">
            ...
            {user && (
              <div className="hidden md:ml-6 md:flex md:space-x-8">
                <Link href={`/${locale}/dashboard`} className="text-gray-900 hover:text-gray-700 px-3 py-2 text-sm font-medium">
                  {t("nav.dashboard")}
                </Link>
                ...
                <Link href={`/${locale}/requests`} className="text-gray-900 hover:text-gray-700 px-3 py-2 text-sm font-medium relative">
                  {t("nav.requests")}
                  {unapprovedRequests > 0 && (
                    <Badge className="absolute -top-1 -right-1 h-5 w-5 flex items-center justify-center text-xs bg-red-500 text-white rounded-full">
                      {unapprovedRequests > 99 ? '99+' : unapprovedRequests}
                    </Badge>
                  )}
                </Link>
                <Link href={`/${locale}/chat`} className="text-gray-900 hover:text-gray-700 px-3 py-2 text-sm font-medium relative">
                  {t("nav.chat")}
                  {unreadMessages > 0 && (
                    <Badge className="absolute -top-1 -right-1 h-5 w-5 flex items-center justify-center text-xs bg-blue-500 text-white rounded-full">
                ...
                <>
                  ...
                  <div className="flex items-center space-x-3 px-3 py-3 border-b">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {profile && <Badge className={`${TIER_COLORS[profile.tier]} text-xs mt-1`}>{TIER_BENEFITS[profile.tier].name}</Badge>}
                    </div>
                  </div>
                  ...
                  <Link
                    ...
                    className="flex items-center justify-between px-3 py-3 text-base font-medium text-gray-900 hover:bg-gray-50 rounded-md"
                    onClick={() => setIsMobileMenuOpen(false)}
                  >
                  ...
                  >
                    ...
                    {unreadMessages > 0 && (
                      <Badge className="h-5 w-5 flex items-center justify-center text-xs bg-blue-500 text-white rounded-full">
                        {unreadMessages > 99 ? '99+' : unreadMessages}
                      </Badge>
                    )}
                  </Link>
